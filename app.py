...
# ==========================================
# 2. CONFIGURAÇÕES GLOBAIS E APIS
# ==========================================
...
# Definição do Atalho para o Session State (Corrige o NameError)
...
if "notas" not in state:
    state.notas = [
        ...
        },
        {
            ...
            "conteudo": "Dificuldade encontrada: O gráfico de pizza da aba de Finanças quebrava e ficava completamente invisível quando o usuário limpava o histórico de compras, pois o Pandas tentava agrupar um DataFrame vazio por 'Categoria'."
        },
        {
            "id": str(uuid.uuid4()),
...
# Garantia de retrocompatibilidade de IDs únicos
...
/* Transformação Premium do st.radio */
    ...
    if prompt := st.chat_input("Pergunte à Verity sobre a sua rotina ou dados..."):
        ...
        with st.chat_message("assistant"):
            ...
            if GEMINI_API_KEY == "SUA_CHAVE_AQUI" or not GEMINI_API_KEY:
                placeholder_resposta.error("Erro de Configuração: Insira uma Chave de API do Gemini válida na variável 'GEMINI_API_KEY'.")
            else:
                try:
                    with st.spinner("Verity está analisando seus dados e pensando..."):
                        ...
                        chat = client.chats.create(
                            ...
                            config=genai.types.GenerateContentConfig(system_instruction=contexto_sistema, temperature=0.7),
...
def render_notas():
    ...
    if state.notas:
        ...
        for idx, nota in enumerate(state.notas):
            ...
            with col_atual:
                st.markdown(f"<div class='nota-container'><h3 style='color:#a855f7; margin-top:0; margin-bottom:10px;'>{nota['titulo']}</h3><p style='color:#e2e8f0; white-space: pre-wrap; font-size:15px; line-height:1.5;'>{nota['conteudo']}</p></div>", unsafe_allow_html=True)
